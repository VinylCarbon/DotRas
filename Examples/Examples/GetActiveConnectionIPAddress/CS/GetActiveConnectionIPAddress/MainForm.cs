...
namespace DotRas.Samples.GetActiveConnectionIPAddress
{
    public partial class MainForm : Form
    {
        ...
        private void LoadConnectionsComboBox()
        {
            ...
            foreach (RasConnection connection in this.Dialer.GetActiveConnections())
            {
        ...
        private void GetAddressButton_Click(object sender, EventArgs e)
        {
            foreach (RasConnection connection in this.Dialer.GetActiveConnections())
            {
                if (connection.EntryId == (Guid)((ComboBoxItem)this.ConnectionsComboBox.SelectedValue).Value)
                {
        ...
        {
            this.GetAddressButton.Enabled = this.ConnectionsComboBox.SelectedIndex > 1;
        }
    }
}