    ...
    public sealed class RasEntryCollection : RasOwnedCollection<RasPhoneBook, RasEntry>
    {
        ...
        internal void Load()
        {
            ...
            try
            {
                ...
                NativeMethods.RASENTRYNAME[] entries = RasHelper.GetEntryNames(this.Owner);
                if (entries != null && entries.Length > 0)
                {
                    for (int index = 0; index < entries.Length; index++)
                    {
                        ...
                        RasEntry item = RasHelper.GetEntryProperties(this.Owner, entry.name);
                        if (item != null)
                        {
                            this.Add(item);
                        }
        ...
        /// <param name="item">An <typeparamref name="RasEntry"/> to insert.</param>
        protected override void InsertItem(int index, RasEntry item)
        {
            ...
            {
                if (RasHelper.SetEntryProperties(this.Owner, item) && !this.Owner.EnableFileWatcher)
                {
        ...
        protected override void RemoveItem(int index)
        {
            ...
            if (!this.IsInitializing)
            {
                ...
                if (entry != null)
                {
                    if (RasHelper.DeleteEntry(entry.Owner.Path, entry.Name) && !this.Owner.EnableFileWatcher)
                    {
