    ...
    public sealed class RasAutoDialAddressCollection : RasCollection<RasAutoDialAddress>
    {
        ...
        internal void Load()
        {
            try
            {
                ...
                Collection<string> addresses = RasHelper.GetAutoDialAddresses();
                if (addresses != null && addresses.Count > 0)
                {
                    for (int index = 0; index < addresses.Count; index++)
                    {
                        RasAutoDialAddress item = RasHelper.GetAutoDialAddress(addresses[index]);
                        if (item != null)
                        {
                            this.Add(item);
                        }
        ...
        /// <param name="item">An <typeparamref name="RasAutoDialAddress"/> to insert.</param>
        protected override void InsertItem(int index, RasAutoDialAddress item)
        {
            ...
            {
                if (RasHelper.SetAutoDialAddress(item.Address, item.Entries))
                {
        ...
        protected override void RemoveItem(int index)
        {
            if (!this.IsInitializing)
            {
                ...
                if (item != null)
                {
                    if (RasHelper.SetAutoDialAddress(item.Address, null))
                    {
