    ...
    public sealed class RasSubEntryCollection : RasOwnedCollection<RasEntry, RasSubEntry>
    {
        ...
        internal void Load(RasPhoneBook phoneBook, int count)
        {
            ...
            try
            {
                ...
                {
                    RasSubEntry subEntry = RasHelper.GetSubEntryProperties(phoneBook, this.Owner, index);
                    if (subEntry != null)
                    {
                        this.Add(subEntry);
                    }
        ...
        /// <param name="item">An <typeparamref name="TObect"/> to insert.</param>
        ...
        protected override void InsertItem(int index, RasSubEntry item)
        {
            ...
            {
                if (RasHelper.SetSubEntryProperties(this.Owner.Owner, this.Owner, index, item))
                {
                    base.InsertItem(index, item);
                }
            }
        ...
        protected override void RemoveItem(int index)
        {
            ...
            else
            {
                ...
                if (RasHelper.DeleteSubEntry(this.Owner.Owner, this.Owner, index + 2))
                {
                    base.RemoveItem(index);
                }
#else
