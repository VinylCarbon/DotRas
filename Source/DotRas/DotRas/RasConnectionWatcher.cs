...
namespace DotRas
{
    ...
    /// </summary>
    /// <remarks>
    /// The connected event raised by this component will be raised whenever a connection has connected on the machine. If
    /// a valid handle is provided to the <see cref="Handle"/> property, the other events will only receive notifications for that 
    /// handle however the connected event will remain unchanged.
    /// </remarks>
    /// <example>
    ...
    public sealed partial class RasConnectionWatcher : RasComponent, ISupportInitialize
    {
        ...
        private void Register(NativeMethods.RASCN changeType, RasHandle handle)
        {
            ...
            int ret = SafeNativeMethods.RasConnectionNotification(handle, waitObject.SafeWaitHandle, changeType);
        ...
        private void ConnectionStateChanged(object obj, bool timedOut)
        {
            lock (this.lockObject)
            {
                if (this.EnableRaisingEvents)
                {
                    ...
                    ReadOnlyCollection<RasConnection> connections = RasHelper.GetActiveConnections();
